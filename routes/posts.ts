...
app.get('/mongo/:inicio/:final', async (req, res) => {
  ...
  const redisCachedResponse = await Redis.get(
    `mongo-posts-${inicio}-${final}:`
  );

  // Si la hay, la envia al usuario
  if (redisCachedResponse) return res.json(redisCachedResponse);
...
app.get('/sql/:inicio/:final', async (req, res) => {
  ...
  `,
      (err, _, rows) => res.json({ posts: rows, err })
    )
  );
});
